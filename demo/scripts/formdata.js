...
$(function(){
  ...
  $("#userfile").change(function() {  // you can ues 'onchange' here to uplpad automatically after select a file
    ...
    console.log(f)
    $.ajax({
      url: 'http://up-z2.qiniu_test.com',  // Different bucket zone has different upload url, you can get right url by the browser error massage when uploading a file with wrong upload url.
      type: 'POST',
      data: f,
      processData: false,
      contentType: false,
      ...
      success: function(res) {
        ...
        if (res.key && res.key.match(/\.(jpg|jpeg|png|gif)$/)) {
          str += '<img src="http://olhygo6ri.bkt.clouddn.com/' + res.key + '"/>';
        }
